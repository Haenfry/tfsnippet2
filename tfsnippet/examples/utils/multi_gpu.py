...
class MultiGPU(object):
    ...
    def data_parallel(self, batch_size, inputs):
        ...
        # slow path: multi-GPUs
            ...
            with tf.compat.v1.name_scope('data_parallel') as ns:
            ...
            for i, device in enumerate(self.work_devices):
                dev_inputs = []
                with tf.compat.v1.name_scope(ns + 'tower_gpu_{}'.format(i)):
                    for inp in inputs:
    ...
    def maybe_name_scope(self, device):
        ...
        if device == self.main_device:
            ...
            with tf.compat.v1.name_scope('tower_cpu') as ns:
                yield ns
        else:
            gpu_id = self._gpu_devices.index(device)
            with tf.compat.v1.name_scope('tower_gpu_{}'.format(gpu_id)) as ns:
                yield ns
    ...
    def average_grads(self, grads):
        ...
        # slow path: multi-GPUs
            ...
            with tf.device(self.main_device), tf.compat.v1.name_scope('average_grads'):
    ...
    def apply_grads(self, grads, optimizer, global_step=None,
        ...
        with tf.device(self.main_device), tf.compat.v1.name_scope('apply_grads'):
            if control_inputs:
    ...
    def average(self, tensors, batch_size=None):
        ...
        with tf.device(self.main_device), tf.compat.v1.name_scope('average_tensors'):
