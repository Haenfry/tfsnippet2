...
def set_variable_values(variables, values):
    sess = get_default_session_or_error()
    sess.run([tf.compat.v1.assign(v, a) for v, a in zip(variables, values)])
    ...
    def test_summary_writer(self):
        ...
        with TemporaryDirectory() as tempdir:
            with TrainLoop([], max_epoch=2, summary_dir=tempdir,
                           summary_graph=tf.compat.v1.get_default_graph()) as loop:
                ...
                with self.test_session():
                    summary_op = tf.compat.v1.summary.scalar('x', tf.constant(1.23))
        ...
        with TemporaryDirectory() as tempdir:
            sw = tf.compat.v1.summary.FileWriter(tempdir)
