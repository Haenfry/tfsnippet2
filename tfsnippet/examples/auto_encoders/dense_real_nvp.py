...
def q_net(x, posterior_flow, observed=None, n_z=None):
    ...
    with arg_scope([spt.layers.dense],
        ...
        h_x = tf.cast(x, dtype=tf.float32)
...
def coupling_layer_shift_and_scale(x1, n2):
    ...
    shift = spt.layers.dense(
        h, n2, kernel_initializer=tf.compat.v1.zeros_initializer(),
        bias_initializer=tf.compat.v1.zeros_initializer(), name='shift'
    )
    scale = spt.layers.dense(
        h, n2, kernel_initializer=tf.compat.v1.zeros_initializer(),
        bias_initializer=tf.compat.v1.zeros_initializer(), name='scale'
    )
    return shift, scale
...
def main():
    ...
    with tf.compat.v1.name_scope('plotting'):
